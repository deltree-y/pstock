...
from utils.const_def import IS_PRINT_LOAD_DATA_INFO
...
class Stock():
    #si即stockinfo对象,必须传入
    def __init__(self, ts_code, si:StockInfo=None, start_date=None, end_date=None):
        ...
        self._df_raw, self.df_filtered = None, None
        self.is_print_load_info = False #是否打印加载信息

        self.load()
        self.load(if_force_load_from_tushare=True) if self.if_need_re_update() else None
        
        #self.df_filtered为输出给其他模块使用的df,已按照Trade_date升序排列
        self.df_filtered = self.get_df_between(self.start_date, self.end_date)  
        logging.info(f"[{self.name}({self.ts_code})]要求获取[{start_date}]-[{end_date}], 实际获取[{self.df_filtered['trade_date'].min()}]-[{self.df_filtered['trade_date'].max()}], 共<{self.df_filtered.shape[0]}>行") if IS_PRINT_LOAD_DATA_INFO else None

    #根据csv文件是否存在,选择是从文件加载还是从tushare获取
    def load(self, if_force_load_from_tushare=False):
        #如果文件存在且不强制从tushare加载, 则从文件加载
        if os.path.exists(self.ffn) and if_force_load_from_tushare==False:
            self._df_raw = pd.read_csv(self.ffn)
            self._df_raw['trade_date'] = self._df_raw['trade_date'].astype(np.int64)  #确保trade_date为int类型
            self.update_first_last_date()   #更新self.first_date和self.latest_date
            logging.info(f"[{self.name}({self.ts_code})]从<{self.ffn}>读取成功, <{self._df_raw.shape[0]}>行,[{self.first_date}]-[{self.latest_date}]") if self.is_print_load_info else None
        #如果文件不存在或强制从tushare加载, 则从tushare加载
        else:
            self.update_from_tushare()  #从tushare获取数据, 更新self._df_raw
            self.update_first_last_date()
            logging.info(f"[{self.name}({self.ts_code})]从tushare读取成功, 共<{self._df_raw.shape[0]}>行,[{self.first_date}]-[{self.latest_date}]")
            self.save()
    ...
    def update_from_tushare(self, start_date=None, end_date=None):
        ...
        self._df_raw = self.si.get_stock_detail(asset=self.asset, ts_code=self.ts_code, start_date=s_date, end_date=e_date)
        self.update_first_last_date()
        logging.debug(f"[{self.name}({self.ts_code})]加载数据[{s_date}]-[{e_date}]成功, 共<{self._df_raw.shape[0]}>行, 最新日期<{self.latest_date}>")
    ...
    #获取给定日期的数据,输出需要按照升序排列
    def get_df_between(self, start_date, end_date):
        if self._df_raw is not None:
            mask = (self._df_raw['trade_date'].astype(int) >= int(start_date)) & (self._df_raw['trade_date'].astype(int) <= int(end_date))
            ...
            df_between = self._df_raw[mask].copy()
            #df_between["trade_date"] = pd.to_datetime(df_between["trade_date"], format="%Y%m%d")
            return df_between.sort_values(by=['trade_date'], ascending=True).reset_index(drop=True)
        else:
    ...
    def update_first_last_date(self):
        if self._df_raw is not None:
            self.first_date, self.latest_date = int(self._df_raw['trade_date'].min()), int(self._df_raw['trade_date'].max())
    ...
    def save(self):
        ...
        if self._df_raw is not None:
            self._df_raw.to_csv(self.ffn, index=False)
            logging.info(f"[{self.name}({self.ts_code})]写入文件 - [{self.ffn}]成功, <{self._df_raw.shape[0]}>行")
        else:
    ...
    def update_daily_file(self, new_df, date, if_load=True, if_save=True):
        ...
        if new_df.shape[0] == 0:
            ...
            if self._df_raw[self._df_raw['trade_date'].isin([int(date)])].shape[0] == 0:  #还没有此日期的数据，可以添加
                self._df_raw = pd.concat([new_df[new_df['ts_code']==self.ts_code], self._df_raw], ignore_index=True)
            else:   
...
if __name__ == "__main__":
    ...
    print(f"s.df_filtered.head(20):\n{s.df_filtered.head(20)}")
    #s = Stock(ts_code, si)
