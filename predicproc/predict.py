...
class Predict():
    ...
    def print_predict_result(self):
        logging.info(f"Predict y1l label[{self.y1r.get_label()}] pct min/avg/max is <{self.y1r.get_rate_from_label('min')*100:.2f}%%/{self.y1r.get_rate_from_label('avg')*100:.2f}%%/{self.y1r.get_rate_from_label('max')*100:.2f}%%> price is <{self.y1r.get_rate_from_label('min')*self.bp+self.bp:.2f}/{self.y1r.get_rate_from_label('avg')*self.bp+self.bp:.2f}/{self.y1r.get_rate_from_label('max')*self.bp+self.bp:.2f}>")
