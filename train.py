...
def auto_search():
    ...
    p = 2
    dropout_rate = 0.5
    feature_type_list = [FeatureType.T2H10_F55]
    ...
    nb_filters, kernel_size, nb_stacks = [512], [8], [4] # TCN模型参数 - nb_stacks-增大会整体重复残差结构，直接增加模型深度, nb_filters-有多少组专家分别提取不同类型的特征, kernel_size-每个专家一次能看到多长时间的历史窗口
    d_model_list, num_heads_list, ff_dim_list, num_layers_list = [256], [8], [512], [2] # Transformer模型参数 - d_model-增大每个时间步的特征维度, num_heads-增大多头注意力机制的头数, ff_dim-增大前馈神经网络的隐藏层维度, num_layers-增大会增加模型深度
    filters_list, kernel_size_list, conv1d_depth_list = [64,128], [4,8], [4,16]   # Conv1D模型参数 - filters-增大每个卷积层的滤波器数量, kernel_size-增大卷积核大小, depth-增大会增加模型深度
