...
class ResidualBlock(tf.keras.layers.Layer):
    def __init__(self, nb_filters, kernel_size, dilation_rate, dropout_rate=0.1, l2_reg=1e-5):
    ...
    def create_model(self, input_shape):
        ...
        for stack in range(self.nb_stacks):
            for dilation in self.dilations:
                x = ResidualBlock(self.nb_filters, self.kernel_size, dilation, self.dropout_rate, self.l2_reg)(x)
        # 仅取最后一个时间步的输出
        x = x[:, -1, :]
        ...
        #outputs = Dense(NUM_CLASSES, activation='softmax', name='output1')(x)
